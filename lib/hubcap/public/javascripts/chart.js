...
var bars = vis.add(pv.Layout.Stack)
    ...
    .layer.add(pv.Bar)
        ...
        .anchor("top").add(pv.Label)
            .textBaseline("bottom")
            .text(function() { return totals_by_week[this.index]; })
            .visible(function() { 
                var this_layer_index = this.parent.index;
                var last_layer_index = this.parent.parent.layers().length - 1;
                return this_layer_index === last_layer_index &&
                       totals_by_week[this.index] > 0;
            });
